...
public class PlayServiceImpl implements PlayService {
    ...
    public boolean choseAnswer(PlayingRequest playingRequest) {
        ...
        List<UserQuestionEntity> optionalUserQuestionEntity = userQuestionRepository.findUserQuestionEntityByUser_EmailAndQuestion_Id(playingRequest.getEmail(), playingRequest.getQuestion());
        if (!optionalUserQuestionEntity.isEmpty()) {
            return false;
        } else {
            ...
            List<UserQuestionEntity> userQuestionEntities = answers.stream().map(it -> {
                UserQuestionEntity userQuestion = new UserQuestionEntity();
                userQuestion.setScore(it.getScore());
                return userQuestion;
            }).toList();
            answers.forEach(it -> it.addUserQuestion(userQuestionEntities));
    ...
    public void removeAnswer(PlayingRequest playingRequest) {
        List<UserQuestionEntity> userQuestionEntities = userQuestionRepository.findUserQuestionEntityByUser_EmailAndQuestion_Id(playingRequest.getEmail(), playingRequest.getQuestion());
        QuestionEntity questionEntity = questionRepository.findById(playingRequest.getQuestion()).orElseThrow(() -> {
