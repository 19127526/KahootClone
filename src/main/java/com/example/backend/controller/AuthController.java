...
public class AuthController extends BaseController {
    ...
    public ResponseEntity<AuthenticationDto> register(@RequestBody AuthRequest authRequest) {
        return ResponseEntity.status(HttpStatus.ACCEPTED).body(accountService.register(authRequest));
    }

    @PostMapping("validate/otp")
    public ResponseEntity<Boolean> validateAccount(@RequestBody AuthRequest authRequest) {
        return ResponseEntity.status(HttpStatus.CREATED).body(accountService.validateRegisterAccount(authRequest));
    }

    @PostMapping("loginTraditional")
    public ResponseEntity<AuthenticationDto> login(@RequestBody UserDto userDto) {
        return ResponseEntity.status(HttpStatus.OK).body(accountService.loginTraditional(userDto));
    }

    @PostMapping("refreshToken")
    public ResponseEntity<JsonWebToken> refreshToken(@RequestBody JsonWebToken jsonWebToken) {
        return ResponseEntity.status(HttpStatus.ACCEPTED).body(accountService.refreshToken(jsonWebToken.getRefreshToken()));
    }
}
